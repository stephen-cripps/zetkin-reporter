...
```bash
zetkin-serverless-api$ sam build
```
...
```bash
zetkin-serverless-api$ sam local invoke NetCodeWebAPIServerless --event events/event.json
```

The AWS SAM CLI can also emulate your application's API. Use the `sam local start-api` command to run the API locally on port 3000.

```bash
zetkin-serverless-api$ sam local start-api
zetkin-serverless-api$ curl http://localhost:3000/
```
...
```bash
zetkin-serverless-api$ sam build
zetkin-serverless-api$ sam deploy
```
...
```bash
zetkin-serverless-api$ sam logs -n NetCodeWebAPIServerless --stack-name zetkin-serverless-api --tail
```
...
```bash
zetkin-serverless-api$ dotnet test test/ServerlessAPI.Tests/ServerlessAPI.Tests.csproj 
```
...
```bash
sam delete --stack-name zetkin-serverless-api
```
